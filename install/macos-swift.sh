...
# Sparkle is a link dependency now (macos-swift/Package.swift), so the app will
# not launch unless its framework rides inside the bundle at the rpath the
# binary was linked with (@executable_path/../Frameworks). A checkout copy
# never STARTS Sparkle — it keeps updating itself with git, see
# Distribution.swift — but it must still carry the framework it links against.
# `ditto` preserves the Versions/ symlink layout a plain `cp -R` can mangle;
# rm+re-ditto (like the binary above) gives a new inode so the running
# instance keeps its old mapped pages until launchd swaps it out. The
# `codesign --deep` below re-signs the framework and its XPC services with the
# rest of the bundle. Never fatal for the same reason the icon is not: this is
# also the update-apply step.
...
if [[ -d "$BUILD_DIR/Sparkle.framework" ]]; then
  ...
  echo "WARNING: Sparkle.framework not found beside the binary — the app may" \
       "fail to launch. A clean 'swift build' should produce it." >&2
fi
