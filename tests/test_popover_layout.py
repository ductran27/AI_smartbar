...
def guard_report(**changes):
    report = {
        ...
        "scope": "local Claude Code sessions in this Linux/WSL environment",
        "claudeVersion": "2.1.0",
        "activeManagedSource": "drop-in",
        "policyPath": ("/etc/claude-code/managed-settings.d/"
                       "99-ai-smartbar-auto-fallback-guard.json"),
        "details": ["Static policy is complete; run live verification."],
        "lastLiveCheck": None,
    }
...
class TestFallbackGuardLayout(unittest.TestCase):
    """Pure status/action/geometry contract for the Linux guard card."""

    def test_runner_states_map_to_the_five_visible_statuses(self):
        cases = [
            ({"state": "protected"}, ("Protected", "good", "verify")),
            ({"state": "protected_inconclusive"},
             ("Protected + inconclusive", "caution", "verify")),
            ({"state": "not_protected"},
             ("Not protected", "neutral", "enable")),
            ({"state": "action_needed", "safetyAutoFallback": "unknown"},
             ("Action needed", "warning", "enable")),
            ({"state": "error", "safetyAutoFallback": "unknown"},
             ("Action needed", "warning", "enable")),
            ({"state": "unsupported", "safetyAutoFallback": "unknown"},
             ("Action needed", "warning", "enable")),
        ]
        for changes, expected in cases:
            with self.subTest(state=changes["state"]):
                self.assertEqual(layout.fallback_guard_presentation(
                    guard_report(**changes)), expected)
        self.assertEqual(layout.fallback_guard_presentation(
            guard_report(state="action_needed")),
            ("Action needed", "warning", "verify"))
        self.assertEqual(layout.fallback_guard_presentation(
            guard_report(), busy="verify"),
            ("Verifying…", "neutral", ""))
        self.assertEqual(layout.fallback_guard_presentation(
            guard_report(), error="refresh failed"),
            ("Action needed", "warning", "verify"))

    def test_explicit_none_still_shows_status_below_the_header_without_usage(self):
        built = layout.build(None, fallback_guard=None, now=NOW)
        self.assertIn("Auto fallback", labels(built))
        self.assertIn("Checking…", labels(built))
        guard_box = next(b for b in boxes(built) if b.fill == t.CARD_BG)
        title = next(s for s in built.shapes
    ...
    def test_omitting_guard_preserves_the_historical_layout(self):
        ...
        self.assertFalse(any(h.name.startswith("fallback-") for h in built.hits))

    def test_protect_and_verify_are_mutually_exclusive(self):
        unprotected = layout.build(
            snap(), fallback_guard=guard_report(state="not_protected",
                                                protected=False), now=NOW)
        protected = layout.build(
            snap(), fallback_guard=guard_report(state="protected"), now=NOW)
        self.assertIn("fallback-enable", {h.name for h in unprotected.hits})
        self.assertNotIn("fallback-verify", {h.name for h in unprotected.hits})
        self.assertIn("fallback-verify", {h.name for h in protected.hits})
        self.assertNotIn("fallback-enable", {h.name for h in protected.hits})

    def test_busy_status_has_no_action_target(self):
        built = layout.build(snap(), fallback_guard=guard_report(),
                             fallback_busy="enable", now=NOW)
        self.assertIn("Installing…", labels(built))
        self.assertFalse(any(h.name in ("fallback-enable", "fallback-verify")
                             for h in built.hits))

    def test_transport_error_is_visible_when_details_expand(self):
        built = layout.build(
            snap(), fallback_guard=None, fallback_expanded=True,
            fallback_error="Fallback guard failed: pkexec unavailable",
            now=NOW)
        self.assertIn("Action needed", labels(built))
        self.assertIn("Fallback guard failed: pkexec unavailable", labels(built))

    def test_manual_opus_availability_requires_live_probe_evidence(self):
        unverified = layout.build(
            snap(), fallback_guard=guard_report(), fallback_expanded=True,
            now=NOW)
        self.assertIn("Manual Opus: Not restricted by guard", labels(unverified))
        live = {
            "status": "passed", "checkedAt": "2026-07-31T12:00:00Z",
            "totalCostUsd": 0.031, "budgetLimitUsd": 0.25,
            "probes": [{"name": "manual_opus", "outcome": "OPUS_OK",
                        "observedModels": ["opus"]}],
        }
        verified = layout.build(
            snap(), fallback_guard=guard_report(lastLiveCheck=live),
            fallback_expanded=True, now=NOW)
        self.assertIn("Manual Opus: Available (live verified)", labels(verified))
        self.assertTrue(any(text.startswith(
            "Live check: Passed · $0.031 · $0.25 limit")
                            for text in labels(verified)))
        self.assertIn("manual_opus: OPUS_OK · opus", labels(verified))
        self.assertIn("Managed source: drop-in", labels(verified))
        unknown = layout.build(
            snap(), fallback_guard=guard_report(
                manualOpusRestrictedByGuard=None), fallback_expanded=True,
            now=NOW)
        self.assertIn("Manual Opus: Unknown", labels(unknown))

    def test_remove_is_advanced_and_requires_a_second_confirm_hit(self):
        collapsed = layout.build(
            snap(), fallback_guard=guard_report(), fallback_expanded=True,
            now=NOW)
        self.assertIn("fallback-advanced", {h.name for h in collapsed.hits})
        self.assertNotIn("fallback-remove", {h.name for h in collapsed.hits})

        advanced = layout.build(
            snap(), fallback_guard=guard_report(), fallback_expanded=True,
            fallback_advanced=True, now=NOW)
        self.assertIn("fallback-remove", {h.name for h in advanced.hits})
        self.assertNotIn("fallback-confirm-remove",
                         {h.name for h in advanced.hits})

        confirm = layout.build(
            snap(), fallback_guard=guard_report(), fallback_expanded=True,
            fallback_advanced=True, fallback_remove_confirm=True, now=NOW)
        names = {h.name for h in confirm.hits}
        self.assertIn("fallback-confirm-remove", names)
        self.assertIn("fallback-cancel-remove", names)
        self.assertNotIn("fallback-remove", names)

    def test_action_button_wins_over_the_card_toggle_hit(self):
        built = layout.build(snap(), fallback_guard=guard_report(), now=NOW)
        action = next(h for h in built.hits if h.name == "fallback-verify")
    ...
    def test_provider_tabs_are_evenly_spaced_between_guard_and_card(self):
        ...
        built = layout.build(both, fallback_guard=guard_report(), now=NOW)
