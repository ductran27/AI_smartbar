...
class TestCardContent(unittest.TestCase):
    ...
    def test_the_rail_never_reflows_the_row_beside_it(self):
        """The rail is drawn INTO the card's own horizontal padding rather
        than shifting inner_l, so becoming the active account cannot move
        anything. Pinned because insetting content to "make room" for it is
        the obvious next edit, and it would make every row on the active
        card a different width from the same row on every other one."""
        active = layout.build(snap(account(number=2, active=True)), now=NOW)
        inactive = layout.build(snap(account(number=2)), now=NOW)
    ...
    def test_a_scoped_label_too_long_for_the_column_truncates_from_the_tail(self):
        ...
        label = next(s for s in built.shapes
                     if isinstance(s, t.Label) and s.text == "Bengalfox")
        self.assertEqual(label.mode, "tail")
...
class TestRemoveAffordance(unittest.TestCase):
    ...
    def test_the_confirm_question_never_needs_more_than_its_reserved_lines(self):
        ...
        self.assertEqual(
            label.text, "Remove nguyen.tran@employmenthero.com · 20x?")
